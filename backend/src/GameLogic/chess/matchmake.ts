...
export default function matchmake(ws : WS) {
  if (threeMinQeue.length >= 2) {
    if (threeMinQeue[0].user._id.equals(threeMinQeue[1].user._id)) return
  ...
  if (fiveMinQeue.length >= 2) {
    if (fiveMinQeue[0].user._id.equals(fiveMinQeue[1].user._id)) return
  ...
  if (tenMinQeue.length >= 2) {
    if (tenMinQeue[0].user._id.equals(tenMinQeue[1].user._id)) return
